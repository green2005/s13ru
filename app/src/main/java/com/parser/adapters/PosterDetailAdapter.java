...
public class PosterDetailAdapter extends SimpleCursorAdapter{
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (viewType == PosterDetailDBHelper.POSTER_RECORD_TYPE.DESCRIPTION.ordinal()){
            convertView = getDescriptionView(convertView, cursor);
        } else
        if (viewType == PosterDetailDBHelper.POSTER_RECORD_TYPE.IMAGE_ATTACHMENT.ordinal()){
            convertView = getImageAttachmentView(convertView, cursor);
        } else
        if (viewType == PosterDetailDBHelper.POSTER_RECORD_TYPE.TIMEPLACE_RECORD.ordinal()){
    ...
    private View getDescriptionView(View convertView, Cursor cursor){
        ...
        tvText.setText(CursorHelper.getString(cursor, PosterDetailDBHelper.TEXT_COLUMN));
        return convertView;
    }
